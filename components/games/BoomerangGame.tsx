    ...
    function initGame() {
      ...
      function renderSel() {
        ...
        for (const key in TYPES) {
          ...
          btn.style.cssText = `flex:1;min-width:100px;font-size:12px;padding:7px 4px;display:flex;flex-direction:column;align-items:center;gap:3px;${key === selType ? 'border:2px solid ' + (key === 'white' ? '#888780' : T.color) + ';' : ''}`;
          btn.disabled = birdState === 'flying';
          btn.innerHTML = `<span style="display:flex;align-items:center;gap:5px;"><span style="width:13px;height:13px;border-radius:50%;background:${T.color};border:1px solid ${T.dark};display:inline-block;"></span><span style="font-weight:500;">${T.name}</span></span><span style="font-size:11px;color:#666;">${T.desc}</span>`;
